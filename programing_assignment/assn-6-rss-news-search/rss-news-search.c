...
typedef struct {
  ...
  Semaphore simultaneousConnections;
  hashset serverConnections;
  Semaphore indicesLock;
  Semaphore previouslySeenArticlesLock;
  Semaphore serverConnectionsLock;
} rssDatabase;
...
static void FreeSemaphores(rssDatabase *db);
...
static void ParseArticle(rssDatabase *db, char *articleTitle, char *articleURL);
static void ScanArticle(streamtokenizer *st, int articleID, hashset *indices,
                        hashset *stopWords, Semaphore indicesLock);
...
static int ServerConnectionHash(const void *elem, int numBuckets);
static int ServerConnectionCompare(const void *elem1, const void *elem2);
static void ServerConnectionFree(void *elem);
...
static const char *const kWelcomeTextFile = "http://localhost:8080/welcome.txt";
static const char *const kDefaultStopWordsFile = "http://localhost:8080/stop-words.txt";
static const char *const kDefaultFeedsFile = "http://localhost:8080/rss-feeds.txt";
...
static const int kMaxSimultaneousConnections = 24;
static const int kNumServerConnectionsBuckets = 1007;
static void SetupSemaphores(rssDatabase *db)
{
  db->simultaneousConnections = SemaphoreNew("Simultaneous Connections",
                                             kMaxSimultaneousConnections);
  db->indicesLock = SemaphoreNew("indices Lock", 1);
  db->previouslySeenArticlesLock = SemaphoreNew("previouslySeenArticles Lock",
                                                1);
  HashSetNew(&db->serverConnections, sizeof(serverConnection),
             kNumServerConnectionsBuckets, ServerConnectionHash,
             ServerConnectionCompare, ServerConnectionFree);
  db->serverConnectionsLock = SemaphoreNew("serverConnections Lock", 1);
}

static void FreeSemaphores(rssDatabase *db)
{
  SemaphoreFree(db->simultaneousConnections);
  SemaphoreFree(db->indicesLock);
  SemaphoreFree(db->previouslySeenArticlesLock);
  HashSetDispose(&db->serverConnections);
  SemaphoreFree(db->serverConnectionsLock);
}
...
static void ProcessEndTag(void *userData, const char *name)
{
  ...
  if (strcasecmp(name, "item") == 0) {
    ThreadNew(entry->url, ParseArticle, 3, state->db, strdup(entry->title),
              strdup(entry->url));
  }
}
...
static const int maxServerConnections = 6;
...
static void ParseArticle(rssDatabase *db, char *articleTitle, char *articleURL)
{
  ...
  SemaphoreWait(db->previouslySeenArticlesLock);
  if (VectorSearch(&db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false) >= 0) {
    SemaphoreSignal(db->previouslySeenArticlesLock);
  ...
  serverConnection sc = { u.serverName };
  SemaphoreWait(db->serverConnectionsLock);
  serverConnection *existingSc = HashSetLookup(&db->serverConnections, &sc);
  if (existingSc == NULL) {
    sc.server = strdup(u.serverName);
    sc.connections = SemaphoreNew(u.serverName, maxServerConnections);
    HashSetEnter(&db->serverConnections, &sc);
    existingSc = HashSetLookup(&db->serverConnections, &sc);
  }
  SemaphoreSignal(db->serverConnectionsLock);

  SemaphoreWait(existingSc->connections);

  SemaphoreWait(db->simultaneousConnections);
  URLConnectionNew(&urlconn, &u);
  switch (urlconn.responseCode) {
      case 0: printf("Unable to connect to \"%s\".  Domain name or IP address is nonexistent.\n", articleURL);
              break;
      case 200: SemaphoreWait(db->previouslySeenArticlesLock);
                // check again if article already seen
                if (VectorSearch(&db->previouslySeenArticles, &newsArticle, NewsArticleCompare, 0, false) >= 0) {
                  SemaphoreSignal(db->previouslySeenArticlesLock);
                  printf("[Ignoring \"%s\": we've seen it before.]\n", articleTitle);
                  break;
                ...
                ScanArticle(&st, articleID, &db->indices, &db->stopWords, db->indicesLock);
                STDispose(&st);
                break;
      case 301: 
      case 302: // just pretend we have the redirected URL all along, though index using the new URL and not the old one...
                SemaphoreSignal(db->simultaneousConnections);
                SemaphoreSignal(existingSc->connections);
                ParseArticle(db, strdup(articleTitle), strdup(urlconn.newUrl));
                SemaphoreWait(existingSc->connections);
                SemaphoreWait(db->simultaneousConnections);
                break;
 ...
 * @param indicesLock a binary lock for indices.
 *
 * No return value.
 */
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      if (WordIsWorthIndexing(word, stopWords)) {
        SemaphoreWait(indicesLock);
        AddWordToIndices(indices, word, articleID);
        SemaphoreSignal(indicesLock);
      }
    }
  }
}
...
static int ServerConnectionHash(const void *elem, int numBuckets)
{
  const serverConnection *sc = elem;
  return StringHash(&sc->server, numBuckets);
}

static int ServerConnectionCompare(const void *elem1, const void *elem2)
{
  const serverConnection *sc1 = elem1;
  const serverConnection *sc2 = elem2;
  return StringCompare(&sc1->server, &sc2->server);
}

static void ServerConnectionFree(void *elem)
{
  serverConnection *sc = elem;
  StringFree(&sc->server);
  SemaphoreFree(sc->connections);
}
