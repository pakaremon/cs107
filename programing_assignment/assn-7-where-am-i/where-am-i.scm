...
(define (partition pivot ls comp)
  (if (null? ls) '(() ())
      (let ((split-of-rest (partition pivot (cdr ls) comp)))
        (if (comp (car ls) pivot)
...
;; -----------------------------
;; Takes a list of circles and returns a list of all the
;; points where the circles intersect.
;;

(define (intersection-points circle-list)
  (if (< (length circle-list) 2) '()
      (append (intersect (car circle-list) (cadr circle-list))
              (intersection-points (cons (car circle-list) (cddr circle-list)))
              (intersection-points (cons (cadr circle-list) (cddr circle-list))))))
              
;;
;; Function: distance-product
;; --------------------------
;; Takes a point and a list of points and returns the
;; product of the distances between that point and all the
;; points in the list.
;;
...
;; ---------------------
;; Takes a list of points and returns a list where each
;; point is annotated to show its distance-product from the
;; other points.
;;

(define (rate-points pt-list)
  (map (lambda (pt) (list (distance-product pt pt-list) pt)) pt-list))

;;
;; Function: sort-points
;; ---------------------
;; Takes a list of rated points, and sorts them in ascending
;; order of rating.
;;

(define (sort-points rpt-list)
  (quicksort rpt-list (lambda (rpt1 rpt2) (< (car rpt1) (car rpt2)))))

;;
;; Function: clumped-points
;; ------------------------
;; Takes a list of points, rates them, sorts them, and then
;; returns the half of the points with the smallest ratings.
;;

(define (clumped-points pt-list)
  (map cadr (prefix-of-list (sort-points (rate-points pt-list))
                            (quotient (length pt-list) 2))))

;;
;; Function: average-point
;; -----------------------
;; Takes a list of points and averages them all down to a
;; single point.
;;

(define (average-point pt-list)
  (let ((pt (list (/ (apply + (map x pt-list)) (length pt-list))
                  (/ (apply + (map y pt-list)) (length pt-list)))))
    (list (distance-product pt pt-list) pt)))

;;
;; Function: best-estimate
;; -----------------------
;; Takes a guess (a list of circles), computes all the
;; points of intersection, winnows those points down to
;; those which are most clumped, and returns their average
;; point.
;;
...
;; Function: where-am-i
;; --------------------
;; Given a list of distances and a list of star locations,
;; computes all the possible guesses, uses best-estimate to
;; get an answer out of each one, and sorts the estimates in
;; increasing order of distance rating. The result is a list
;; of rated points.
;;

(define (where-am-i distances stars)
  (prefix-of-list (sort-points (map best-estimate (all-guesses distances stars))) 5))
